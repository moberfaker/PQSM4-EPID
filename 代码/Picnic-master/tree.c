...
void expandSeeds(tree_t* tree, uint8_t* salt, size_t repIndex, paramset_t* params)
{
    ...
    for (size_t i = 0; i <= lastNonLeaf; i++) {                 // 拓展所有已知的结点的子结点（所有）
        ...
        hashSeed(tmp, tree->nodes[i], salt, HASH_PREFIX_1, repIndex, i, params);        // tmp = H(nodes[i] || salt || repIndex || nodeIndex)
...
tree_t* generateSeeds(size_t nSeeds, uint8_t* rootSeed, uint8_t* salt, size_t repIndex, paramset_t* params)
{
    ...
    expandSeeds(tree, salt, repIndex, params);                  // 拓展所有已知的结点的子结点（所有）
...
int hasSibling(tree_t* tree, size_t node)   // 是否有兄弟结点
{
    if (!exists(tree, node)) {      // 树不存在该结点返回0
        return 0;
    }

    if (isLeftChild(node) && !exists(tree, node + 1)) {     // node是左子结点，且node+1不存在，返回0
        return 0;
    }
...
size_t* getRevealedNodes(tree_t* tree, uint16_t* hideList, size_t hideListSize, size_t* outputSize)
{
    /* 计算从hideList到root的路径，存储为节点集 */
    size_t pathLen = tree->depth - 1;

    /* pathSets[i][0...hideListSize] 为hideListSize中的每个叶节点存储深度为i的路径中的节点 */
    size_t** pathSets = malloc(pathLen * sizeof(size_t*));
    size_t* slab = malloc(hideListSize * pathLen * sizeof(size_t));

    for (size_t i = 0; i < pathLen; i++) {
        pathSets[i] = slab;                 // pathSets[i] 指向地址 slab[hideList * i]
        slab += hideListSize;
    }
    ...
    for (size_t i = 0; i < hideListSize; i++) {
        size_t pos = 0;
        size_t node = hideList[i] + (tree->numNodes - tree->numLeaves); /* 输入列表叶索引，转换为节点 */
        pathSets[pos][i] = node;
        pos++;
    ...
    for (size_t d = 0; d < pathLen; d++) {
        for (size_t i = 0; i < hideListSize; i++) {
            if (!hasSibling(tree, pathSets[d][i])) {                            // 不存在兄弟结点
                continue;
            }
            size_t sibling = getSibling(tree, pathSets[d][i]);                  // 兄弟结点
            if (!contains(pathSets[d], hideListSize, sibling )) {               // contains(list, len, value)，不包含sibling进入if代码块
                // Determine the seed to reveal 
                while(!hasRightChild(tree, sibling) && !isLeafNode(tree, sibling)) {    // 没有右子结点且不是叶结点，进入while代码块
                    sibling = 2 * sibling + 1; // sibling = leftChild(sibling)
                }

                // Only reveal if we haven't already 
                if (!contains(revealed, revealedPos, sibling)) {                // revealed中不包含sibling，则进入if代码块
