...
int EPID_Member_Join_fake(Msgs* message, GP_MANAGER_CTX* ctxgp_Pool)
{
	...
	uint32_t Index_in_X = ctxgp->MemSize;
	Msgsmemcpy(message, &Memberpk, X_EACH_SIZE_BYTE);
	Msgsmemcpy(message, &Index_in_X, X_INDEX_BYTE);
	Msgsmemcpy(message, &signature_len, sizeof(size_t));
	Msgsmemcpy(message, ctxgp->ComList.XSig, signature_len);
...
void EPID_Manager_updaet_cert(Msgs* message, GP_MANAGER_CTX* ctxgp_Pool) {
	...
	//检查是否为群成员
	uint32_t Index_in_X = find_in_X(&Memberpk, 1, ctxgp->ComList.X, ctxgp->MemSize);
	if (Index_in_X == -1) {
...
int EPID_Verify(uint8_t** text, Msgs* message, size_t Member_size, uint8_t* X)
{
	...
	uint8_t* temp = message->message;
	memcpy(&text_len, temp, sizeof(size_t));
	*text = (uint8_t*)malloc(text_len);
	temp += sizeof(size_t);
	memcpy(*text, temp, text_len);
	memset(*text + text_len, 0x00, 1);
	temp += text_len;
	memcpy(&signature_len, temp, sizeof(size_t));
	signature = (uint8_t*)malloc(signature_len);
	temp += sizeof(size_t);
	memcpy(signature, temp, signature_len);

	int ret = EPID_MemberProof_verify(signature, signature_len, *text, text_len, Member_size, X);
	if (ret == -1) {
		return -1;
	}
	return 0;
