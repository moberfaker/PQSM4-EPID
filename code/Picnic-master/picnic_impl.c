...
{
    setBit((uint8_t*)array, bitNumber, val);    // Set a specific bit in a byte array to a given value
}
...
/*
    查S盒的函数B
    参数:	uint32_t b
    返回值:	查S盒的结果uint32_t b
*/
uint32_t Readsbox(uint32_t b) {
    b = (Sbox[(uint8_t)(b >> 24)] << 24) + (Sbox[(uint8_t)(b >> 16)] << 16) + (Sbox[(uint8_t)(b >> 8)] << 8) + Sbox[(uint8_t)b];
    return b;
}

/*
    循环左移算法
    参数：	uint32_t a    length：循环左移位数
    返回值：uint32_t b
*/
...
/*
    合成变换T
    参数：	uint32_t a    short mode：1表示明文的T，调用L；2表示密钥的T，调用L'
    返回值：合成变换后的uint32_t a
*/
uint32_t Tchange(uint32_t a, short mode) {
    return mode == 1 ? L1(Readsbox(a)) : L2(Readsbox(a));
}
...
/*
    ...
    返回值：无
*/
void InitKey(uint32_t MK[], uint32_t K[]) {
    int i;
    for (i = 0; i < 4; i++) {
        K[i] = MK[i];
    }
...
/*
    ...
    返回值：无
*/
void ExtendKey(uint32_t RK[], uint32_t K[]) {
    short i;
    for (i = 0; i < 32; i++) {
        K[(i + 4) % 4] = K[i % 4] ^ Tchange(K[(i + 1) % 4] ^ K[(i + 2) % 4] ^ K[(i + 3) % 4] ^ CK[i], 2);
        RK[i] = K[(i + 4) % 4];
    }
}
...
/*
    ...
    返回值：无
*/
void GenerateKey(uint32_t MK[], uint32_t K[], uint32_t RK[]) {
    InitKey(MK, K);
    for (int i = 0; i < 4; i++) {
        printHex("OURS", &K[i], 4);
    }
...
/*
    反转函数
    参数；	uint32_t X[4]：反转对象    uint32_t Y[4]：反转结果
    返回值：无
*/
void Reverse(uint32_t X[], uint32_t Y[]) {
    short i;
    for (i = 0; i < 4; i++) {
        Y[i] = X[4 - 1 - i];
    }
...
void SM4Enc(uint32_t* plaintext, uint32_t* ciphertext, uint32_t* Key) {
    ...
    //    printf("[%2d]：", i);
    //    for (int j = 0; j < 4; j++) {
    //        printf("%02X",((uint8_t*)&roundKey[i])[j]);
    //    }
    //    printf("    ");
    //    
    //    if (i % 4 == 3)	printf("\n");
    ...
    //printf("***********生成密文*******************\n");
    for (i = 0; i < 4; i++)
        plaintxt[i] = plaintext[i];

    for (i = 0; i < 32; i++) {
        plaintxt[(i + 4) % 4] = plaintxt[i % 4] ^ Tchange(plaintxt[(i + 1) % 4] ^ plaintxt[(i + 2) % 4] ^ plaintxt[(i + 3) % 4] ^ roundKey[i], 1);
        //printf("%d\t", i);
        //printHex("plaintext[]", &plaintxt[(i + 4) % 4], 4);
    }
    Reverse(plaintxt, ciphertext);
