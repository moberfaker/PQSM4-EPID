...
size_t MemberProof_size(int MemberSize)
{
    ...
    size_t OmproofSize = paramset.seedSizeBytes * (ceil_log2(paramset.numMPCParties) + 1) // Info to recompute seeds(and 1 more seed for N's resume part mask )
        + paramset.OmAndSizeBytes + paramset.stateSizeBytes * 2             // msgs -> [s] of unopened party + output_masks
...
void allocateSignatureMem(signatureMem_t* sig, Memparamset_t* params)
{
    ...
    //uint8_t* slab = calloc(1, params->numMPCRounds * (params->memberSize * sizeof(ReproofMem_t) + sizeof(ReproofMem_t*)));
    //sig->Reproofs = (ReproofMem_t**)slab;
    //slab += params->numMPCRounds * sizeof(ReproofMem_t*);
    //for (size_t i = 0; i < params->numMPCRounds; i++) {
    //    sig->Reproofs[i] = (ReproofMem_t*)slab;
    //    slab += params->memberSize * sizeof(ReproofMem_t);
    //}
}
...
static void commitMem(uint8_t* digest, uint8_t* seed, uint8_t* aux, uint8_t* salt, size_t t, size_t j, Memparamset_t* params,size_t ReorOm)
{
    ...
    HashInitMem(&ctx, params, HASH_PREFIX_NONE);
 ...
 */
static void computeAuxTapeMem(randomTape_t* tapes, Memparamset_t* params,size_t REorOm)
{
    ...
    randomTape_t* tapes, msgs_t* msgs, Memparamset_t* params, size_t ReorOm, uint32_t* Zinter,uint32_t* pubkey,uint8_t Pre_Challenge)
{
    ...
    if (ReorOm != 0 && memcmp(output, pubkey, params->stateSizeBytes) != 0) {       // 如果output和ciphertext不相等，Exit
        ...
        //reconstructShares(xx, Plaintext_mask);
        //printHex("output_mask", (uint8_t*)xx, 16);
        ...
        //reconstructShares((uint32_t*)xx, Key_mask);
        //printHex("key_mask", (uint8_t*)xx, 16);
        //printf("\n");
...
int MemberProof_sign(Memparamset_t* paramset, MemKey* Key, const uint8_t* message, size_t message_len,
    uint8_t* signature, size_t* signature_len, uint8_t* Xlist,size_t X_index) {
    //check if in the X
    ...
    ret = sign_memberpf(paramset, (uint32_t*)Key->sk, (uint32_t*)Key->pk, (uint32_t*)Key->c, message, message_len, sig, Xlist, X_index);
...
int sign_memberpf(Memparamset_t* params, uint32_t* sk, uint32_t* pk, uint32_t* plaintext,
    const uint8_t* message, size_t message_len, signatureMem_t* sig, uint8_t* Xlist, size_t X_index)
{
    int ret = 0;
    ...
    tree_t* iRootSeedsTree = generateSeedsMem(params->numMPCRounds, saltAndRootAndRemask + params->saltSizeBytes + params->seedSizeBytes, sig->salt, 0, params);
    tree_t* iRemaskSeedsTree = generateSeedsMem(params->numMPCRounds, sig->RemaskRootSeed, sig->salt, 0, params);
    uint8_t** iRootSeeds = getLeaves(iRootSeedsTree);
    uint8_t** iRemaskSeeds = getLeaves(iRemaskSeedsTree);
    free(saltAndRootAndRemask);

    //tapes[MPCRounds][memberSize+1][64]
    tree_t** iSeedsTree = malloc(params->numMPCRounds * sizeof(tree_t*));       //343 
    MemTree_t* SeedsTree = allocateMemTree(params);  //343->N+1
    randomTape_t** tapes = allocateTape_Pointer(params);                        //343->N+1->64

    //RemaskTapes[MPCRounds][memberSize]
    tree_t** RemaskSeedsTree = malloc(params->numMPCRounds * sizeof(tree_t*));
    RemaskTape_t* Remasktapes = malloc(params->numMPCRounds * sizeof(RemaskTape_t));//343->N
...
#pragma omp for schedule(guided)
        for (t = 0; t < params->numMPCRounds; t++) {
            ...
            for (N = 0; N < params->memberSize + 1; N++) {
                SeedsTree[t][N] = generateSeedsMem(params->numMPCParties, iSeeds[N], sig->salt, N, params);//343->N+1
...
#pragma omp for schedule(guided)
        for (t = 0; t < params->numMPCRounds; t++) {
            computeAuxTapeMem(&tapes[t][0], params, 0);
        }
    }
...
#pragma omp for schedule(guided)
    ...
    inputs_t inputs = allocateInputsMem(params);
    Zinter_t Zinter = allocateZinter(params);

    msgs_t** msgs = malloc((params->memberSize + 1) * sizeof(msgs_t*));
    shares_t** Key_mask = malloc(params->numMPCRounds * sizeof(shares_t*));
    shares_t** Plaintext_mask = malloc(params->numMPCRounds * sizeof(shares_t*));
...
#pragma omp for schedule(guided)
        for (t = 0; t < params->numMPCRounds; t++) {
            uint8_t* inter_mask = malloc(params->stateSizeBytes * 2);
            reconstructShares(inter_mask, Key_mask[t]);
            reconstructShares(inter_mask + params->stateSizeBytes, Plaintext_mask[t]);
            for (N = 1; N <= params->memberSize; N++) {
                GetZinterTape(Remasktapes[t].tape[N - 1], inter_mask, &tapes[t][N], params);
                computeAuxTapeMem(&tapes[t][N], params, N);
            }
            free(inter_mask);
        }
    }
...
#pragma omp for schedule(guided)
    ...
    /* 运行resume part的在线阶段 */
    //uint32_t xxx[4];
    //printHex("Xlist ", Xlist, params->stateSizeBytes * params->memberSize * 2);
    //printHex("pubkey", pk, params->stateSizeBytes);
        ...
        for (t = 0; t < params->numMPCRounds; t++) {
            if (contains(challengeC, params->numOpenedRounds, t)) {
                ...
                for (size_t N = 1; N < params->memberSize + 1; N++) {
                    ...
                    memcpy(Reproof[t].msgs[N - 1], msgs[N][t].msgs[hideList[0]], params->ReAndSizeBytes);
